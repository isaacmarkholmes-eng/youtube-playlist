...
async function init() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Could not load videos.json (${response.status})`);
    }

    const data = await response.json();
    if (!Array.isArray(data) || data.length === 0) {
      throw new Error("videos.json must contain at least one video.");
    }

    videos = data.filter((entry) => entry && entry.id);
    if (!videos.length) {
      throw new Error("No valid video IDs found in videos.json.");
    }
